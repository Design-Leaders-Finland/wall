...
class AppInitializationService {
  ...
  static Future<void> initializeApp() async {
    ...
    try {
      ...
      final sslTest = await SSLCertificateService.testSSLConnection(
        supabaseUrl,
      );

      if (!sslTest) {
        AppLogger.warning(
          'SSL connection test failed, but proceeding with initialization',
        );
      } else {
      ...
      AppLogger.info(
        'Supabase initialized with SSL certificate and enforced SSL connections',
      );
      ...
      if (elapsedMilliseconds < minimumSplashDuration) {
        ...
        AppLogger.info(
          'Waiting additional $remainingTime ms to meet minimum splash duration',
        );
